...
log_entry()
{
    if grep -qi ^"$1" storage/logs/docker-winter.log ; then
        REPLACEMENT=$@
        sed -i "s#$1.*#$REPLACEMENT#g" storage/logs/docker-winter.log
    else
        echo "$@" >> storage/logs/docker-winter.log
    fi
    return 0
}
...
plugins_changed()
{
    ...
    CURRENT_PLUGINS="$(grep -i "Plugins installed:" storage/logs/docker-winter.log | sed 's/^.*:\s //')"
...
themes_changed()
{
    ...
    CURRENT_THEMES="$(grep -i "Themes installed:" storage/logs/docker-winter.log | sed 's/^.*:\s //')"
...
touch storage/logs/docker-winter.log
...
if config_wants_database ; then
    ...
    if [ $DB_UP -eq 1 ] ; then
      ...
      if ! grep -qi '^Plugins last installed:' storage/logs/docker-winter.log || [ $CMS_REDO_PLUGINS = "true"  ]; then
          install_plugins
      fi
      ...
      if ! grep -qi '^Themes last installed:' storage/logs/docker-winter.log || [ "$CMS_REDO_THEMES" = "true"  ]; then
          install_themes
      fi
